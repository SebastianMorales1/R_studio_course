...
#con los corchetes cuadrados se identifica un valor o valores en las coordenadas expresadas
#With the square brackets, we can identify the value(s) of the specified coordinates.
#Con los corchetes se extraen elementos de la misma clase.
mi_vector[3]
#Se puede extraer una secuencia de valores.
#We can identify a sequence of values.
#Los índices en R comienzan desde 1.
#The index in R starts from 1.
mi_vector[1:5]
# no tiene que ser estrictamente secuencial
# It does not have to be strictly sequential
mi_vector[c(4,6,13)]
# No tiene que tener un orden en particular
# It does not have to have a particular order
mi_vector[c(6,13,4)]

#Extrae elementos de una lista o dataframe donde no hay necesariamente elementos con la misma clase.
#It shows elements from a list or dataframe that not necessarily are of the same class.
mi_vector[[3]]

#Para quitar elementos en una funcion usar numeros negativos
#To take off elements in a function, we can use negative numbers
mi_vector[-9:-15]

#$ extrae elementos de un data frame con nombres literales.
help("$")

#Se puede usar el vector lógico, que muestra los valores que cumplen la condición.
#It can be used a logical vector, that shows the values that achieve the condition.
mi_vector[mi_vector > 15]

#Se crea un vector logico de secuencia verdadero y falso con longitud 10.
...
#Subconjunto de datos de una matrix
#Subsetting of matrix

#matrix[x,y] valor de coordenada especifica/ value of a particular coordenate
mi_matriz[2,2]
#matrix[x, ] valor de una fila entera o varias / to show an entire row
mi_matriz[2, ]
mi_matriz[2:3,]
#We can chose some particular rows or columns
mi_matriz[c(1,3),]
mi_matriz[,c(1,3)]
#matrix[ ,y] valor de una columna entera / to show an entire column
mi_matriz[ ,2]
mi_matriz[ ,2:3]
# Da error si pedimos filas o columnas que no existen en la matriz.
mi_matriz[5, ]
...
#Se pueden mostrar las coordenadas 1 y 3 de la lista "y".
#We can show the coordenates 1 and 3 from the list "y".
y <- list(hello1=2:7, hello2=0.9, hello3="puente")
print(y[c(1,3)])
#lista[[]] pueden servir como índices computados.
#lista[[]] can be useful as computed index.
name <- "hello3"
y[[name]]
y$name #No funciono para "$"/ it did not work with "$"
y$hello3

#Subsetting de elementos anidados de una lista.
#Subsseting of nested elementos of a list.
z <- list(b = list(10,11,12), b = c(9.8,2.33))
#En ambas notaciones se extraen la lista uno y de esta lista el elemento 3.
#In both notations, it is chosen the first list "1", and from this first list its thir element "3".
z[[c(1,3)]]
z[[1]][[3]]
#Se extraen la segunda lista, y el primer valor
#Similarly to the last case, it is chosen the second list and from it the first value.
z[[c(2,1)]]

#Emparejado parcial/ partial matched
#Si no encuentra al simbolo identico, escoge el mas cercano.
#IF R does not finde the identical symbol, it chooses the most similar.
w <- list(mmmbmbmbm = 1:35)
w[["m"]]#Sale NULL, it shows NULL
w$m #Sale completo/ it shows the full list

#¿Cómo quitar valores faltantes de un vector?.
#How to take off the NA values of a vector?
v <- c(321,53,NA,NA,NA,34)
faltantes <- is.na(v)
#vector sin datos faltantes NA/ vector without NA values :)
v[!faltantes] 

#Generar con ella un vector lógico.
